...
Uses odd chromosomes for training, even chromosomes for evaluation
...
def main():
    ...
    parser.add_argument("--out-eval", required=True,
                        help="Output evaluation/calibration table (TSV)")
    ...
    # Split by chromosome: odd = train, even = test
    train = [r for r in data if chrom_number(r["chrom"]) is not None
             and chrom_number(r["chrom"]) % 2 == 1]
    test = [r for r in data if chrom_number(r["chrom"]) is not None
            and chrom_number(r["chrom"]) % 2 == 0]

    print(f"  Train (odd chroms):  {len(train):,} intervals, "
          f"{sum(r['length'] for r in train):,} bases", file=sys.stderr)
    print(f"  Test  (even chroms): {len(test):,} intervals, "
          f"{sum(r['length'] for r in test):,} bases", file=sys.stderr)

    # Evaluate hand-tuned model first (baseline)
    print("\nEvaluating hand-tuned model...", file=sys.stderr)
    ht_train = evaluate_handtuned(train, "Hand-tuned (train)")
    ht_test = evaluate_handtuned(test, "Hand-tuned (test)")
    print_eval(ht_train)
    print_eval(ht_test)
    ...
    lr_train = evaluate(train, weights, "Calibrated (train)")
    lr_test = evaluate(test, weights, "Calibrated (test)")
    print_eval(lr_train)
    print_eval(lr_test)

    # Comparison summary
    print(f"\n{'='*60}", file=sys.stderr)
    print(f"Summary comparison", file=sys.stderr)
    print(f"{'='*60}", file=sys.stderr)
    print(f"  {'Model':<25} {'Log-loss (test)':>15} {'AUC (test)':>12}",
          file=sys.stderr)
    print(f"  {'-'*25} {'-'*15} {'-'*12}", file=sys.stderr)
    print(f"  {'Hand-tuned':<25} {ht_test['log_loss']:>15.6f} "
          f"{ht_test['auc']:>12.4f}", file=sys.stderr)
    print(f"  {'Calibrated (logistic)':<25} {lr_test['log_loss']:>15.6f} "
          f"{lr_test['auc']:>12.4f}", file=sys.stderr)

    improvement = (ht_test['log_loss'] - lr_test['log_loss'])
    pct = 100 * improvement / ht_test['log_loss'] if ht_test['log_loss'] > 0 \
        else 0
    print(f"\n  Log-loss improvement: {improvement:+.6f} ({pct:+.1f}%)",
          file=sys.stderr)
    ...
    model = {
        ...
        "training": {
            ...
            "chromosomes": "odd (1,3,5,...,21)",
            "log_loss": lr_train["log_loss"],
        },
        "test": {
            "n_intervals": len(test),
            "n_bases": sum(r["length"] for r in test),
            "chromosomes": "even (2,4,6,...,22)",
            "log_loss": lr_test["log_loss"],
            "auc": lr_test["auc"],
        },
        "baseline": {
            "model": "hand-tuned",
            "log_loss": ht_test["log_loss"],
            "auc": ht_test["auc"],
        },
    }
    ...
    # Save evaluation table (both models, test set)
    with open(args.out_eval, "w") as f:
        f.write("model\tbin_lo\tbin_hi\tn_intervals\tn_bases\t"
                "pred_mean\tobs_mean\n")
        for c in ht_test["calibration"]:
            f.write(f"hand_tuned\t{c['bin_lo']:.2f}\t{c['bin_hi']:.2f}\t"
                    f"{c['n_intervals']}\t{c['n_bases']}\t"
                    f"{c['pred_mean']:.6f}\t{c['obs_mean']:.6f}\n")
        for c in lr_test["calibration"]:
            f.write(f"calibrated\t{c['bin_lo']:.2f}\t{c['bin_hi']:.2f}\t"
                    f"{c['n_intervals']}\t{c['n_bases']}\t"
                    f"{c['pred_mean']:.6f}\t{c['obs_mean']:.6f}\n")
    print(f"Evaluation table saved to {args.out_eval}", file=sys.stderr)
